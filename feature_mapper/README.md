# Tutorial: Implementing the `barycenter_mapper` script
...
> [!WARNING]
> This tutorial doesn't cover comprehensively the Pure Data part. Some [useful links](#useful-links) can be found at the end of the tutorial

The Pure Data file defines a default melody to which several filters can be applied. It is configured to wait for 2 OSC signals from the `tempo` and `resonance` routes. 
...
> [!TIP]
> It's possible to control any filter by either connecting the output of an OSC route to the input of the filter to control, or, as illustrated in this tutorial, by creating another route bound to another feature coming from the main program.
...
``` python
    def process_features(self, raw_landmarker_data):
    ...
    def _compute_barycenter_1D(self, coordinates):
